...
16. **[ARCH / SP4.5] `emPanel` engine-registration port (`run_panel_cycles` Rust-only divergence)** — surfaced 2026-04-18 during SP5 brainstorming as an explicit out-of-scope carve-out of the SP5 work. SP5 closed the per-view notice-dispatch divergence but deliberately did *not* touch `run_panel_cycles`, because the underlying cause is a separate structural gap.
    ...
    **SP5 divergences to review before choosing Option A vs B.** SP5 made three concrete departures from its own original plan; they are recorded here because SP4.5's Option A is structurally isomorphic to SP5 and will likely face the same decisions. Read the C++ source + the SP5 diff at `47efa6b` before committing to an SP4.5 approach.

    1. **Ring ownership stayed on `PanelTree`, not `emView`.** The plan specified the `notice_ring_head_next/prev` + `has_pending_notices` fields on `emView` to mirror C++ `emView::NoticeList` at `emView.h:576`. Implementation kept them on `PanelTree` because `RefCell` borrow rules make ring-on-`emView` infeasible: `emView::HandleNotice` takes `&mut self` for the duration of the drain, so any panel callback that fires a notice during dispatch would have to re-borrow the view through the `Rc<RefCell<emView>>` — runtime panic. Storing the ring on `PanelTree` (which is passed as `&mut PanelTree` alongside `&mut self`) sidesteps the re-borrow entirely; callbacks that fire notices mutate the tree directly, and the drain's `while let Some(id) = tree.notice_ring_head_next` picks up newly-queued panels as the ring extends. The annotation at `emPanelTree.rs:~297` cites `emView.h:576`, classifies as forced, and affirms that per-view dispatch semantics (each view using its own `CurrentPixelTallness`) are preserved by the 1:1 view/tree ownership invariant every production construction site honors. Wake-up plumbing uses `view_rc.try_borrow_mut()` inside `PanelTree::add_to_notice_list` — on failure (re-entrant during dispatch), ring linkage still proceeds but the engine-wake is skipped (the engine is already awake).

       **SP4.5 implication for Option A.** If `cycle_list` moves to `emView`, any panel callback that schedules more cycling during `emView::Update`'s drain will face the same re-borrow problem. Expect to land `cycle_list` on `PanelTree` with a DIVERGED annotation of the same shape, unless the panel-cycle callback set is provably non-re-entrant (which it probably is not, given C++ allows panels to re-schedule themselves).

    2. **`init_panel_view` is Rust-only plumbing.** The `PanelData::View: Weak<RefCell<emView>>` back-reference must be written *after* the `Rc<RefCell<emView>>` exists, but the `emView` itself needs a root `PanelId` that only exists after `PanelTree::create_root_deferred_view` returns. SP5 added `PanelTree::init_panel_view(id, weak)` (plus a `#[cfg(test)] set_panel_view` wrapper that propagates to descendants) to close the resulting chicken-and-egg. No C++ analogue — C++ passes the view as a reference at construction because it owns the view, not a `Weak`. Marked with `// DIVERGED:` explaining it's Rust-only.

       **SP4.5 implication.** If panels register themselves for cycling with a view back-reference, Option A will likely need an analogous two-step init or a parameter plumbed through panel construction. Precedent here.

    3. **`PanelData::View` exposed as `view()` snake_case accessor.** C++ uses `GetView()` (field is `emPanel::View &`). SP5's Rust method is `view()` — a stylistic rename with `// DIVERGED:` citing the C++ name. Consider whether SP4.5's cycle-registration API should follow the same convention (snake_case accessor, DIVERGED-annotated) or stick closer to C++ (`GetCycleSlot()`/similar). Affects SP4.5's API shape, not its behavior.

    **SP5 fix landed alongside that SP4.5 should inherit.** `SetFocused` and `set_active_panel` were writing `panel.pending_notices` via `tree.get_mut()` + an obsolete `mark_notices_pending()` bool-setter — no ring enrollment, no wake-up. The framework-side global safety-net scan masked this pre-SP5. SP5 removed that global fallback and route both call sites through `tree.queue_notice(id, flags)`. If SP4.5 touches analogous "write panel state without going through the queue" patterns for cycle registration, the same pitfall applies: always route through the authoritative enqueue path so the engine actually wakes.
