...
| Tests | 2440/2440 nextest, 9 skipped, 0 failed (2 new in SP5: `panel_data_view_defaults_none` field-default test, `sp5_per_view_notice_dispatch_uses_correct_pixel_tallness` multi-view dispatch test; 6 new in SP4.5: `sp4_5_panel_engine_registered_at_init_panel_view`, `sp4_5_child_panel_engine_registered_via_init_propagation`, `sp4_5_panel_engine_deregistered_on_panel_removal`, `sp4_5_register_pending_engines_catches_late_scheduler_attach`, `sp4_5_panel_cycle_uses_per_view_pixel_tallness`, `sp4_5_wake_up_panel_from_cycle_reaches_sibling`) |
...
| Known Rust-port incompletenesses remaining | SP8 (sub-view synchronous-settlement divergence — `emSubViewPanel::Paint` drives `run_panel_cycles` synchronously; surfaced 2026-04-19 during SP4.5), W3 surface de-dup (SP6, optional), `emContext` threading (SP7, ARCH). SP1 (animator-forward DIVERGED, re-entrancy docs, W4 polish), SP2 (`InvalidateHighlight` audit — landed in SP1 as W1b), SP3 (CoreConfig ownership), SP4 (`emView::Update` engine-only routing + Phase-8 single-engine test), SP4.5 (`emPanel` engine-registration via `PanelCycleEngine` adapter), and SP5 (per-view notice dispatch) all closed (SP1–SP5 on 2026-04-18; SP4.5 on 2026-04-19). |
...
| `DIVERGED:` (new this wave+W3+W4+SP1+SP3+SP5+SP4.5) | 12 | 1 W3 (`OsSurface` accessor inlining) + 4 W4 (arity-overload renames; `VisitByIdentityBare`/`SetGoalCoords` post-SP1) + 1 SP1 (`emView::Input` animator-forward location) + 2 SP3 (`emCoreConfig::VISIT_SPEED_MAX`/`VisitSpeed_GetMaxValue` flattening; `emSubViewPanel` standalone config) + 3 SP5 (notice-ring ownership on `PanelTree` vs C++'s `emView`; `init_panel_view` Rust-only init helper; `PanelData::View` → `view()` snake_case accessor rename) + 1 SP4.5 (PanelCycleEngine adapter: emPanel→emEngine single-inheritance + slot-map ownership forced divergence); all warranted per CLAUDE.md |
...
| **SP4.5 — `emPanel` engine-registration port (`PanelCycleEngine` adapter)** | 16 | **Complete 2026-04-19** (branch `sp4.5-empanel-engine-registration`; merge commit TBD). | `specs/2026-04-19-emview-sp4-5-empanel-engine-registration-design.md`, `plans/2026-04-19-emview-sp4-5-empanel-engine-registration.md` |
...
| **SP8 — Sub-view synchronous-settlement divergence** | 17 | Not started; ARCH; surfaced 2026-04-19 during SP4.5 implementation — `emSubViewPanel` drives settlement (cycling + HandleNotice + Update + animator) synchronously inside Paint via the sub-tree's `run_panel_cycles`. Sub-view has no scheduler attached, so SP4.5 could not migrate this path to `PanelCycleEngine` without a sub-project of its own. Also keeps `PanelTree::cycle_list` / `Cycle` / `cancel_cycle` / `run_panel_cycles` alive past SP4.5. | — |

**Suggested execution order:** ~~SP1~~ → ~~SP3~~ → ~~SP4~~ → ~~SP5~~ → ~~SP4.5~~ → SP8 (new) → SP6 if wanted → SP7 when the motivation arrives. (SP2 turned out to be already done — landed in SP1 as W1b.) SP1, SP3, SP4, and SP5 landed 2026-04-18; SP4.5 landed 2026-04-19.
...
17. **[ARCH / SP8] Sub-view synchronous-settlement divergence** — surfaced 2026-04-19 during SP4.5 implementation (Phase 4 grep pass). SP4.5 planned to delete `PanelTree::cycle_list` / `Cycle` / `cancel_cycle` / `run_panel_cycles` once top-level panel cycling moved through the scheduler via `PanelCycleEngine`; the grep turned up two additional callers the original plan missed, and the user narrowed SP4.5 scope rather than expanding it.
