...
| Known Rust-port incompletenesses remaining | `InvalidateHighlight` call sites (SP2), `emView::Update` scheduler re-entrant borrow + Phase-8 test promotion (SP4), per-view notice dispatch (SP5, blocked on multi-window roadmap), W3 surface de-dup (SP6, optional). SP1 (animator-forward DIVERGED, re-entrancy docs, W4 polish) and SP3 (CoreConfig ownership) closed 2026-04-18. |
