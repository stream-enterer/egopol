...
| emView.rs:1832 | scheduler | borrow_mut | nested-from-Update | vulnerable | `RawVisitAbs` popup-creation branch calls `sched.borrow_mut()` directly (line 1832). Reachable from `Update` (`emView.rs:2497` SVPChoiceInvalid branch → `RawVisitAbs`) which is called from `UpdateEngineClass::Cycle` (`emView.rs:263`) while `DoTimeSlice` holds `scheduler.borrow_mut()`. Re-entrant scheduler borrow panics. |
