...
- **DrawOp parameter diff**: Compare C++ vs Rust paint call parameters op-by-op. Record Rust ops: `DUMP_DRAW_OPS=1 cargo test --test golden <name> -- --test-threads=1`. C++ ops generated by `make -C tests/golden/gen run`. Diff: `python3 scripts/diff_draw_ops.py <name>`. JSONL files in `target/golden-divergence/`. Use this to diagnose golden failures mechanically — do not form hypotheses from code reading.
